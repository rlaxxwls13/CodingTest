...
2. 프로그래머스 뒤에 있는 큰 수 찾기 다시 풀어보기

## Tip
1. map 객체는 iterator이기 때문에 한 번 사용하면 내부 데이터를 소모하여 다시 사용할 수 없음. 재사용 하려면 list 객체로 변환해야 함
2. 반올림: `round(num, (반올림할 자리수))`, `math.ceil()`, `math.floor()` (`import math` 필수)
...
7. 딕셔너리 정렬: `dict(sorted(d.items(), key = lambda x:x[정렬원하는index], reverse=True(내림차순))`
